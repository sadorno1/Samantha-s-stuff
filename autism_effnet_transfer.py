...
def load_backbone_weights(model: nn.Module, checkpoint_path: str) -> None:
    """
    Loads a RAF-DB-trained checkpoint into the model, skipping any layers
    whose shapes do not match (especially the final classifier).
    """
...
@torch.no_grad()
def benchmark_inference(model: nn.Module, device: torch.device, input_shape=(1, 3, 224, 224), iters: int = 200, warmup: int = 30):
    model.eval()
...
def run_experiment(
    ...
    if use_rafdb_init:
        if not args.rafdb_checkpoint:
            raise ValueError("You set use_rafdb_init=True but did not provide --rafdb_checkpoint")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if score > best_score:
            ...
            torch.save(best_state, f"{exp_name.replace(' ', '_').lower()}_best.pth")
...
def main():
    ...
    parser.add_argument("--rafdb_checkpoint", type=str, default=None,
                        help="Path to best EfficientNet-B0 checkpoint trained on RAF-DB")
    ...
    results["EffNet-B0 (ImageNet only)"] = run_experiment(
        ...
        args=args,
        device=device,
        use_rafdb_init=False,
    )

    # RAF-DB initialized
    results["EffNet-B0 (RAF-DB initialized)"] = run_experiment(
    ...
    print("\n" + "=" * 110)
    print("SUMMARY")
    header = f"{'Experiment':32s}  {'Acc':>6s}  {'MacroF1':>7s}  {'BestEp':>6s}  {'Params(M)':>9s}  {'Size(MB)':>8s}  {'ms/img':>7s}"
    print(header)
    print("-" * len(header))
    for name, r in results.items():
        print(f"{name:32s}  {r.acc:6.4f}  {r.macro_f1:7.4f}  {r.best_epoch:6d}  {r.params_m:9.2f}  {r.size_mb:8.1f}  {r.latency_ms:7.2f}")
